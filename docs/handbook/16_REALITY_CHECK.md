...
| Multi-tenant isolation (KAN-33) | The real tenancy boundary — `user_id`-scoped ownership checks, 404-not-403 to close the IDOR-existence-oracle gap — is now independently verified end-to-end for `workflows.py` (12 tests, including the approve/reject actions that gate real GitHub writes), `agent_runs.py` (7 tests, including the standalone-planning-context path KAN-9 also covers), and the one per-user-sensitive path in `knowledge.py` — `GitHubConnection` delete/health, the rest of that router being intentionally admin-shared config (5 tests). 24 tests total, all passing. | `organization_id` — what `ARCHITECTURE.md` previously described as the enforcement mechanism — **does not exist anywhere in the codebase** (now corrected there); the verified `user_id` mechanism is a per-router convention, not a structural guarantee. 3 of 26 routers swept. The sweep also surfaced a real, unresolved finding: `engineering_sessions.py` has **no ownership check at all** — any authenticated user can read/mutate any Engineering Session — filed as KAN-44, blocked on a product decision (private per-investigator vs. intentionally shared team workspace) rather than guessed at. |
