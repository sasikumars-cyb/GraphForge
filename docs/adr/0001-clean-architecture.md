...
## Context
GraphForge's backend needs to support several external integrations over time (GitHub, Jira, Neo4j, an AI analysis engine) without those integrations being decided at day one. The initial scaffold has no business logic — the structural decision has to hold up before there's any code to prove it wrong.
