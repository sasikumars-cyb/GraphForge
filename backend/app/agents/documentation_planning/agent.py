...
# Development/Testing/Engineering Review (via create_llm_provider()),
# separate error class per the existing per-agent convention.
# ---------------------------------------------------------------------------
...
async def _call_llm(user_prompt: str, model: str | None = None) -> str:
    try:
        ...
        error = DocumentationPlanningLLMError(exc.message)
...
def _parse_llm_response(raw: str, goal: str) -> DocumentationPlan:
    try:
        ...
        raise DocumentationPlanningLLMError(f"LLM response is not valid JSON: {exc}") from exc
