...
async def _resolve_pull_request_url_subject(db: AsyncSession, user_id: uuid.UUID, url: str) -> Subject:
    ...
    (same ownership scoping as `_resolve_repository_subject`, needed here
    to know which stored GitHub access token to fetch the PR with). The
    ...
    if pull_request is not None:
        return resolve_pr_subject(pull_request.id, display_name=f"{owner}/{repo_name}#{number}: {pull_request.title}")
