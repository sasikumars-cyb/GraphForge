            ...
            <div className="min-w-0 flex-1">
              <p
                className={`text-sm font-semibold ${
                  stage.status === "pending" ? "text-slate-500" : "text-slate-100"
                }`}
              >
                {label}
              </p>
              ...
              {stage.status === "pending" && (
                <p className="mt-0.5 text-xs italic text-slate-600">Waiting…</p>
              )}
              ...
              {lines.length > 0 && (
                <ul className="mt-1.5 flex flex-col gap-1">
                  {lines.map((line, i) => (
                    ...
                    >
                      <span className={line.failed ? "text-rose-400" : "text-emerald-400"}>
                        {line.failed ? "✕" : "✓"}
                      </span>
                      <span className={line.failed ? "text-rose-300" : "text-slate-300"}>
                        {line.text}
                      </span>
                    </li>
                  ))}
              ...
              {stage.status === "running" && (
                <p className="mt-1.5 flex items-center gap-1.5 text-xs text-sky-300">
                  <span className="inline-flex gap-0.5">
                    <Dot delay={0} />
                    <Dot delay={150} />
                    <Dot delay={300} />
                  </span>
                  {lines.length > 0 ? "Working…" : "Starting up…"}
                </p>
              )}
            </div>
          </div>
...
function StageGlyph({ status }: { status: WorkflowStageInfo["status"] }) {
  ...
  if (status === "failed") return <XCircle className="h-4 w-4 text-rose-400" aria-hidden="true" />;
  return <Clock className="h-4 w-4 text-slate-600" aria-hidden="true" />;
}
